...
import { UserPlus, Trash2, ArrowLeft, Download, Upload, Users, Edit2, Check, X, RefreshCcw, AlertTriangle, ArrowRight, Trophy } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900 font-sans relative">
      ...
      <div className="max-w-6xl mx-auto px-4 py-12 relative">
        ...
        {/* CABECERA Y NAVEGACIÓN INTELIGENTE */}
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
          ...
          {selectedCategory ? (
            <button onClick={() => { setSelectedCategory(''); setSelectedSchool(''); router.replace('/admin/inscripcion', { scroll: false }); }} className="p-4 bg-white border border-slate-200 rounded-2xl text-slate-500 hover:text-slate-900 hover:bg-slate-50 transition-all flex items-center gap-2 text-xs font-black uppercase tracking-widest shadow-sm">
        ...
        {!selectedCategory && !selectedSport && (
           <div className="space-y-6 animate-in fade-in">
             <h2 className="text-2xl font-black text-slate-900 uppercase tracking-widest flex items-center gap-2 mb-6">
               <Trophy className="text-blue-600" size={24}/> Selecciona el Deporte
             </h2>
               ...
               {categories.filter(c => c.sports?.name === selectedSport).map(c => (
                 <button 
                   ...
                   onClick={() => {
                     ...
                     router.replace(`/admin/inscripcion?cat=${c.id}`, { scroll: false });
                   }}
                 ...
                 >
                   ...
                   <div className="mt-8 flex items-center text-[10px] font-black text-slate-500 uppercase tracking-widest group-hover:text-blue-600 w-full justify-between">
                     Gestionar Nóminas <ArrowRight size={14} className="group-hover:translate-x-2 transition-transform" />
                   </div>
                 </button>
               ))}
             </div>
        ...
        {selectedCategory && (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
            ...
            <div className="bg-white border border-slate-200 p-6 rounded-[2rem] mb-8 shadow-sm relative overflow-hidden">
              ...
              <label className="text-[10px] text-slate-500 uppercase font-black tracking-[0.2em] mb-3 flex items-center gap-2 mt-2"><Users size={14} className="text-blue-500"/> Seleccionar Delegación / Colegio</label>
              <select className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl text-sm font-bold text-slate-900 outline-none focus:border-blue-400 focus:bg-white transition-colors cursor-pointer" value={selectedSchool} onChange={(e) => setSelectedSchool(e.target.value)}>
                <option value="">Seleccionar colegio inscrito...</option>
