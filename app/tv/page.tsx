...
export default function TvLobbyPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-900 text-white font-sans flex flex-col relative overflow-hidden">
      ...
      <div className="relative z-20 w-full px-6 py-4 border-b border-slate-800 bg-slate-900/50 backdrop-blur-sm flex justify-between items-center">
      ...
      <div className="max-w-6xl mx-auto w-full px-6 py-8 relative z-10 flex-1 flex flex-col">
        
        <div className="flex items-center justify-between mb-12 border-b border-slate-800 pb-6">
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(255,255,255,0.1)] p-2">
              <img src="/logo.png" alt="Logo" className="w-full h-full object-contain" />
            </div>
            <div>
              <h1 className="text-3xl font-black uppercase tracking-widest flex items-center gap-3">
                <MonitorPlay className="text-blue-500" size={32} /> Central de Transmisión
              </h1>
        ...
        {loading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-500">
            <MonitorPlay size={80} className="mb-6 opacity-20" />
            <h2 className="text-2xl font-black uppercase tracking-widest text-slate-400">No hay partidos en vivo</h2>
            <p className="text-sm font-medium mt-2">El operador de la Mesa de Control debe "Arrancar" un partido para que aparezca aquí.</p>
          </div>
        ) : (
            ...
            {liveMatches.map(match => {
              ...
              return (
                <Link 
                  key={match.id}
                  href={`/tv/${match.id}`}
                  className="group bg-slate-800/50 border border-slate-700 p-8 rounded-[2rem] hover:bg-slate-800 hover:border-blue-500 transition-all flex flex-col shadow-xl backdrop-blur-sm"
                >
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                      {getSportIcon(sportName, 24)}
                      <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{sportName}</span>
                    </div>
                    <span className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse">
                      Live
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-4 mb-8">
                    <div className="flex flex-col items-center gap-3 w-1/3">
                      <div className="w-16 h-16 bg-white rounded-2xl p-2">
                        <img src={match.home_team?.schools?.logo_url} className="w-full h-full object-contain" />
                      </div>
                      <span className="text-xs font-black uppercase text-center line-clamp-2">{match.home_team?.name}</span>
                    </div>

                    <div className="flex flex-col items-center justify-center w-1/3">
                      <span className="text-3xl font-black tabular-nums">{match.home_score} - {match.away_score}</span>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{match.current_period}</span>
                    </div>

                    <div className="flex flex-col items-center gap-3 w-1/3">
                      <div className="w-16 h-16 bg-white rounded-2xl p-2">
                        <img src={match.away_team?.schools?.logo_url} className="w-full h-full object-contain" />
                      </div>
