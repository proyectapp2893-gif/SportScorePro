...
import 'server-only';
import { remainingSuspension } from '../discipline/suspension';
import { compareTeamsForStandings, getMatchScoreForStandings, getResultPoints, getSportRules } from '../sports/rules';
import { normalizeDoubleCautions } from '../discipline/double-caution';
...
export function stampSuspensionOrigins(snapshot:BulletinSnapshot,bulletinNumber:number,previous?:BulletinSnapshot):BulletinSnapshot {
  return {
    ...
    categories: snapshot.categories.map(category => ({
      ...category,
      sanctions: category.sanctions.map((sanction:any) => {
        const old = previous?.categories.find(item => item.id === category.id)?.sanctions.find((item:any) => item.id === sanction.id);
        const originalMatches = Number(old?.originalMatches ?? old?.matches ?? sanction.matches ?? 0);
...
export async function buildBulletinSnapshot(db:any, tournamentId:string, asOfDate?:string|null):Promise<BulletinSnapshot> {
  ...
  return {categories:categories.map((category:any)=>{
    ...
    const normalizedCardIds=new Set(normalizedCards.map(event=>event.id));
    const supersededCardIds=categoryEvents.filter((event:any)=>event.event_type==='YELLOW'&&!normalizedCardIds.has(event.id)).map((event:any)=>event.id);
    ...
    return {supersededCardIds,id:category.id,name:category.name,sport:category.sports?.name||'Deporte',fairPlayEnabled:Boolean(tournament?.fair_play_enabled),round,
