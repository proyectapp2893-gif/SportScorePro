...
export default function MatchSheetsPage() {
  ...
  {loading ? <p className="py-20 text-center text-xs font-black uppercase tracking-widest text-blue-600">Cargando planillas...</p> : !categoryId ? <div className="rounded-3xl border border-amber-200 bg-amber-50 p-10 text-center text-sm font-black uppercase text-amber-700">Abre este módulo desde una categoría del panel principal.</div> : <><section className="mb-6 flex flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-2 overflow-x-auto">{phases.map((item) => <button key={item} onClick={() => setPhase(item)} className={`shrink-0 rounded-xl px-5 py-3 text-[10px] font-black uppercase tracking-widest ${phase === item ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{item}</button>)}</div><button onClick={() => download(data.matches, 'planillas-todas-las-jornadas.pdf')} className="flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-[10px] font-black uppercase tracking-widest text-white"><Files size={16}/> Descargar todas las jornadas</button></section>
