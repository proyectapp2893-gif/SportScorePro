...
import { ArrowLeft, CalendarDays, Download, FileDown, House, School } from 'lucide-react';
...
async function drawMatchSheet(pdf: any, match: any, category: any, addPage: boolean) {
  ...
  pdf.setFontSize(8); pdf.text(`${String(category?.tournaments?.name || '').toUpperCase()} · ${String(category?.name || '').toUpperCase()}`, margin, 18);
  pdf.setTextColor(148, 163, 184); pdf.setFontSize(6.5); pdf.text(`${match.matchdays?.scheduled_date || 'FECHA PENDIENTE'} · ${match.scheduled_time?.slice(0, 5) || '--:--'} · ${String(match.venue || 'CANCHA PENDIENTE').toUpperCase()} · JORNADA ${match.matchdays?.round_number || '-'}`, pageWidth - margin, 21, { align: 'right' });
  pdf.setTextColor(255, 255, 255); pdf.setFontSize(9); pdf.text(`${String(match.home_team?.name || 'LOCAL').toUpperCase()}  VS  ${String(match.away_team?.name || 'VISITANTE').toUpperCase()}`, pageWidth - margin, 10, { align: 'right' });
  ...
  const drawRoster = (team: any, startY: number) => {
    ...
    const rows = Math.max(roster.length, 14); const rowHeight = Math.min(5.8, 84 / rows); let y = startY;
    pdf.setFillColor(219, 234, 254); pdf.rect(margin, y, tableWidth, 8, 'F'); pdf.setTextColor(29, 78, 216); pdf.setFontSize(8); pdf.setFont('helvetica', 'bold'); pdf.text(`${String(team?.name || 'EQUIPO').toUpperCase()} · ${roster.length} JUGADORES`, margin + 3, y + 5.3); y += 8;
    ...
    for (let index = 0; index < rows; index += 1) {
      ...
      let valueX = margin; values.forEach((value, valueIndex) => { pdf.setTextColor(15, 23, 42); pdf.setFont('helvetica', valueIndex === 2 ? 'bold' : 'normal'); pdf.setFontSize(Math.min(7.4, Math.max(5.4, rowHeight + 1.1))); if (value) pdf.text(pdf.splitTextToSize(value, widths[valueIndex] - 3).slice(0, 1), valueX + 1.5, y + Math.min(4, rowHeight - 0.8)); valueX += widths[valueIndex]; if (valueIndex < widths.length - 1) pdf.line(valueX, y, valueX, y + rowHeight); }); y += rowHeight;
    }
    return y;
  };
  const homeEnd = drawRoster(match.home_team, 32); const awayEnd = drawRoster(match.away_team, homeEnd + 8);
  pdf.setFont('helvetica', 'bold'); pdf.setTextColor(51, 65, 85); pdf.setFontSize(7); pdf.text('MARCADOR FINAL: LOCAL ______  VISITANTE ______', margin, Math.min(286, awayEnd + 9)); pdf.text('ÁRBITRO: ____________________     FIRMA MESA: ____________________', pageWidth - margin, Math.min(286, awayEnd + 9), { align: 'right' });
}

export default function MatchSheetsPage() {
  const params = useParams(); const searchParams = useSearchParams(); const slug = params.slug as string; const categoryId = searchParams.get('cat') || ''; const asOfDate = normalizeAsOfDate(searchParams.get('hasta'));
  ...
  useEffect(() => {
    ...
    if (slug === DEMO_SLUG) {
      ...
      const matches = db.matches.filter((match) => match.matchdays?.category_id === categoryId && match.status !== 'BYE' && (!asOfDate || match.matchdays?.scheduled_date <= asOfDate)).map((match) => ({ ...match, home_team: { ...match.home_team, players: db.players.filter((player) => player.team_id === match.home_team_id) }, away_team: { ...match.away_team, players: db.players.filter((player) => player.team_id === match.away_team_id) } }));
      setData({ category, matches }); setLoading(false); return;
    }
    loadMatchSheets(slug, categoryId, asOfDate).then((result) => { if (!result.success) toast.error(result.error); else setData(result.data); setLoading(false); });
  }, [slug, categoryId, asOfDate]);
  ...
  const download = async (matches: any[], filename: string) => { if (!matches.length) return toast.error('No hay partidos en esta fase.'); const { jsPDF } = await import('jspdf'); const pdf = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' }); for (let index = 0; index < matches.length; index += 1) await drawMatchSheet(pdf, matches[index], data.category, index > 0); pdf.save(filename); };
  ...
  {loading ? <p className="py-20 text-center text-xs font-black uppercase tracking-widest text-blue-600">Cargando planillas...</p> : !categoryId ? <div className="rounded-3xl border border-amber-200 bg-amber-50 p-10 text-center text-sm font-black uppercase text-amber-700">Abre este módulo desde una categoría del panel principal.</div> : <><section className="mb-6 rounded-3xl border border-slate-200 bg-white p-5"><div className="flex gap-2 overflow-x-auto">{phases.map((item) => <button key={item} onClick={() => setPhase(item)} className={`shrink-0 rounded-xl px-5 py-3 text-[10px] font-black uppercase tracking-widest ${phase === item ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{item}</button>)}</div></section>
  <div className="space-y-6">{Object.entries(phaseMatches.reduce((groups: Record<string, any[]>, match: any) => { const key = `Jornada ${match.matchdays?.round_number || '-'}`; (groups[key] ||= []).push(match); return groups; }, {})).map(([round, matches]) => <section key={round} className="overflow-hidden rounded-3xl border border-slate-200 bg-white"><div className="flex flex-col gap-3 bg-slate-950 px-5 py-4 text-white sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><CalendarDays size={17}/><h2 className="text-sm font-black uppercase tracking-widest">{round}</h2></div><button onClick={() => download(matches as any[], `planillas-${String(round).toLowerCase().replaceAll(' ', '-')}.pdf`)} className="flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-2.5 text-[9px] font-black uppercase tracking-widest text-white hover:bg-emerald-400"><Download size={15}/> Descargar jornada completa</button></div><div className="divide-y divide-slate-100">{(matches as any[]).map((match) => <article key={match.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-slate-50"><School className="text-slate-300"/></div><div className="min-w-0"><p className="font-black uppercase text-slate-900">{match.home_team?.name} <span className="text-slate-300">vs</span> {match.away_team?.name}</p><p className="mt-1 text-[9px] font-bold uppercase tracking-widest text-slate-400">{match.matchdays?.scheduled_date || 'Fecha pendiente'} · {match.scheduled_time?.slice(0,5) || '--:--'} · {match.venue || 'Cancha pendiente'} · {(match.home_team?.players?.length || 0) + (match.away_team?.players?.length || 0)} jugadores</p></div></div><button onClick={() => download([match], `planilla-${match.home_team?.name}-${match.away_team?.name}.pdf`)} className="flex shrink-0 items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-[9px] font-black uppercase tracking-widest text-blue-700"><FileDown size={15}/> Descargar planilla</button></article>)}</div></section>)}</div></>}
  </div></main>;
}
