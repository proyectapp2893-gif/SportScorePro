...
export default function AdminTournamentContext() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (slug === DEMO_SLUG) {
        ...
        const candidate = searchParams.get('tournament') || window.localStorage.getItem(`sportscore:admin-tournament:${slug}`) || db.tournaments[0]?.id || '';
      ...
      const storedId = window.localStorage.getItem(`sportscore:admin-tournament:${slug}`);
      const candidate = (urlId && options.some((item) => item.id === urlId) && urlId) || (storedId && options.some((item) => item.id === storedId) && storedId) || options.find((item) => item.is_active)?.id || options[0]?.id || '';
      setSelectedId(candidate);
      if (candidate) window.localStorage.setItem(`sportscore:admin-tournament:${slug}`, candidate);
    };
    load();
  ...
  const changeTournament = (id: string) => {
    setSelectedId(id);
    window.localStorage.setItem(`sportscore:admin-tournament:${slug}`, id);
    router.push(`/${slug}/admin?tournament=${id}`);
  };
