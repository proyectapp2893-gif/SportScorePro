...
export default async function BulletinsPage({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<{tournament?:string;hasta?:string}>}){
  const {slug}=await params;const {tournament:tournamentId,hasta}=await searchParams;const asOfDate=normalizeAsOfDate(hasta);
