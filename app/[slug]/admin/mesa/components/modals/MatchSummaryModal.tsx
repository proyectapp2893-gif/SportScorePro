    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 sm:p-8 animate-in fade-in duration-300">
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] shadow-2xl flex flex-col overflow-hidden border border-slate-200 relative z-50">
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10 bg-slate-50/50 relative shadow-[inset_0_10px_20px_rgba(0,0,0,0.02)] z-10">
          <div className="mb-8 rounded-2xl border border-blue-100 bg-blue-50 p-4"><p className="text-[10px] font-black uppercase tracking-widest text-blue-700">Jugadores que participaron</p><p className="mt-1 text-xs font-semibold text-slate-600">Marca todos los jugadores que ingresaron al partido. Esta selección se guardará al cerrar el acta.</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{([['HOME', match.home_team, homeRoster, homeParticipants], ['AWAY', match.away_team, awayRoster, awayParticipants] ] as const).map(([team, teamData, roster, selected]) => <div key={team}><p className="mb-2 text-[9px] font-black uppercase text-slate-500">{teamData?.name}</p><div className="grid gap-1">{roster.map((player: any) => <label key={player.id} className="flex cursor-pointer items-center gap-2 rounded-lg bg-white px-3 py-2 text-xs font-bold"><input type="checkbox" checked={selected.includes(player.id)} onChange={() => toggleParticipant(team, player.id)} className="h-4 w-4 accent-blue-600" />#{player.shirt_number || '-'} {player.name}</label>)}</div></div>)}</div></div>
