...
export default async function PublicTeamPage({ params }: { params: Promise<{ slug: string; teamSlug: string }> }) {
  ...
  (events || []).filter((event: any) => event.matches?.status === 'LIVE' || event.matches?.status === 'FINISHED').forEach((event: any) => {
    ...
    if (!scorersById[event.player_id]) scorersById[event.player_id] = { ...event.players, total: 0 };
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8">
      <div className="flex justify-end">
        <Link href={`/${slug}/resultados`} className="flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-5 py-3 text-[10px] font-black uppercase tracking-widest text-blue-700 transition-colors hover:bg-blue-100 sm:w-auto"><BarChart3 size={16} /> Ver resultados generales</Link>
      </div>
