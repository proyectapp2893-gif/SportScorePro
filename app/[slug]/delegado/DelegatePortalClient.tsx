...
export default function DelegatePortalClient({ slug, initialData }: DelegatePortalClientProps) {
  ...
  const disciplinaryBlocks = isDemo ? getDisciplinaryBlocks(data?.eventsByTeam?.[selectedTeam?.id] || [], [], 0) : data?.disciplinaryBlocksByTeam?.[selectedTeam?.id];
  const disciplinaryReason = (playerId: string): string => disciplinaryBlocks == null ? 'No se pudo verificar la habilitación. Actualiza la página.' : disciplinaryBlocks[playerId] || '';
