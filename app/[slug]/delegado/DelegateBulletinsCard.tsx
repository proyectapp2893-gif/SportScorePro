  ...
  return <>
    ...
    {open && createPortal(<div className="fixed inset-0 z-[9999] flex items-stretch justify-center overflow-hidden bg-slate-950/80 p-0 backdrop-blur-sm sm:items-center sm:p-5">
      <section role="dialog" aria-modal="true" aria-labelledby="delegate-bulletins-title" className="flex h-[100dvh] min-h-0 w-full max-w-6xl flex-col overflow-hidden bg-slate-50 shadow-2xl sm:h-[92dvh] sm:rounded-3xl">
        ...
        <div className="flex shrink-0 gap-2 overflow-x-auto border-b border-slate-200 bg-white px-3 py-2.5 sm:p-4">{bulletins.map((bulletin) => <a key={bulletin.id} href={`#delegate-bulletin-${bulletin.id}`} className="shrink-0 rounded-xl bg-indigo-50 px-3 py-2 text-[8px] font-black uppercase tracking-wider text-indigo-700 sm:px-4 sm:py-2.5 sm:text-[9px]">Boletín {bulletin.bulletin_number}</a>)}</div>
        <div className="min-h-0 flex-1 space-y-5 overflow-x-hidden overflow-y-auto overscroll-contain p-2.5 sm:p-6">{bulletins.map((bulletin, index) => <div key={bulletin.id} id={`delegate-bulletin-${bulletin.id}`} className="scroll-mt-4"><p className="mb-2 px-1 text-[9px] font-black uppercase tracking-widest text-slate-500">Boletín {bulletin.bulletin_number} · Información acumulada hasta la Fecha {bulletin.bulletin_number}</p><TournamentBulletinCard snapshot={bulletin.snapshot} number={bulletin.bulletin_number} confirmedAt={bulletin.confirmed_at} initialOpen={index === 0} /></div>)}</div>
      </section>
    </div>, document.body)}
  </>;
}
